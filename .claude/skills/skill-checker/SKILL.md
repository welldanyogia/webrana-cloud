---
name: skill-checker
description: Automatic routing skill that matches tasks to applicable Claude agents and skills based on keywords, patterns, and context. Runs automatically to suggest optimal workflow.
---

# Skill Checker - Automatic Agent/Skill Router

## Purpose

Automatically analyzes task descriptions and routes to the most appropriate Claude agent(s) and skill(s). This skill provides quick routing recommendations without full task analysis.
...
START
  ...
  │
  └─ Unclear/Complex?
      └─ → using-agents skill (full analysis)
```
...
| Secondary | [agent-name] | [if applicable] |
...
### Confidence
- [ ] High (clear single-agent task)
- [ ] Medium (may need coordination)
- [ ] Low (recommend full analysis with using-agents)

### Suggested Action
Use Task tool with `subagent_type: [recommended-agent]`

### Alternative
If task is complex, use `using-agents` skill for full analysis
```
...
## Integration with using-agents

```
skill-checker = Quick routing (< 30 seconds)
using-agents = Full analysis (comprehensive planning)
...
*Fast routing for efficient task delegation. When in doubt, escalate to using-agents for full analysis.*
